...
void LoadTextures() //carga de todas las texturas
{
	...
	frontal = generateTextures("Texturas/frontal1.jpg", 0);
	trasera = generateTextures("Texturas/trasera1.jpg", 0);
	superior= generateTextures("Texturas/superior1.jpg", 0);
	inferior = generateTextures("Texturas/inferior1.jpg", 0);
	lado = generateTextures("Texturas/lados.jpg", 0);
	...
	glBindTexture(GL_TEXTURE_2D, lado);
...
void display(void) //En esta funcion jugamos con todas las luces, a cada tipo de luz se le creo un tipo de shader, pero no por eso se deben manejar los shaders así, se pueden mezclar. Para fines didacticos se hizo así
{
	...
	model = glm::scale(model, glm::vec3(2.0f, 2.0f, 2.0f));//escalamos
	...
	//lightingShader.setMat4("model", model);
	lightingShader.setInt("material_diffuse", lado);
